...
// Center-aligned draw (original)
const drawLineCenter = (ctx, line, x, y, letterSpacing) => {
  const chars = Array.from(line);
...
// Left-aligned draw
const drawLineLeft = (ctx, line, x, y, letterSpacing) => {
  const chars = Array.from(line);
  let cursor = x; // x is the left margin
  chars.forEach((char, index) => {
    ctx.fillText(char, cursor, y);
    cursor += ctx.measureText(char).width + (index === chars.length - 1 ? 0 : letterSpacing);
  });
...
const buildTextCanvas = ({ container, width, height, dpr, props }) => {
  ...
  const isLeft = props.textAlign === 'left';
  ...
  let fontSizePx = parseFloat(computed.fontSize) || 16;
  ...
  if (!Number.isFinite(lineHeight)) {
    lineHeight = fontSizePx * (typeof props.lineHeight === 'number' ? props.lineHeight : 1.4);
  }
  probe.remove();
  ...
  // For left-align: use left margin. For center: use center x.
  const leftMargin = Math.max(4, fontSizePx * 0.1);
  const availableWidth = isLeft ? (width - leftMargin * 2) : width;

  const maxWidth = availableWidth * (isLeft ? 0.98 : 0.86);
  ...
  if (isLeft) {
    lines.forEach((line, index) =>
      drawLineLeft(ctx, line, leftMargin, startY + index * lineHeight, letterSpacing)
    );
  } else {
    lines.forEach((line, index) =>
      drawLineCenter(ctx, line, width / 2, startY + index * lineHeight, letterSpacing)
    );
  }
...
const WarpText = ({
  ...
  const propsRef = useRef({
    text, color, fontSize, fontWeight, fontFamily,
    letterSpacing, lineHeight, textAlign,
    warpStrength, warpScale, speed,
    pointerInfluence, pointerStrength, refraction, ripple
  });
