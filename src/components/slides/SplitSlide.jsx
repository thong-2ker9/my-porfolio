...
export default function SplitSlide({ slide }) {
    ...
    <div data-reveal className="relative">
      <div className="relative overflow-hidden rounded-2xl border border-white/10 shadow-card">
        <img
          ...
          alt={img?.alt}
          loading="lazy"
          className="h-[24rem] w-full object-cover transition-transform duration-700 hover:scale-105 sm:h-[26rem]"
        />
        ...
        {slide.badge && (
          <div className="absolute left-5 top-5 rounded-card border border-white/15 bg-black/60 px-4 py-2.5 backdrop-blur-md">
            ...
            <p className="text-xs text-body/70">{slide.badgeSub}</p>
          </div>
        )}
        {img?.caption && (
  ...
  return (
    <SlideSection id={slide.id} num={slide.num} className="flex items-center py-16 sm:py-20">
      <div
        className={`relative z-10 mx-auto grid w-full max-w-7xl items-center gap-10 px-6 sm:px-10 lg:gap-8 ${
          flip
